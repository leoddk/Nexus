...
const GlobeComponent: React.FC<GlobeProps> = ({ points, onPointClick, selectedPoint }) => {
  ...
  useEffect(() => {
    if (selectedPoint && globeEl.current) {
      globeEl.current.pointOfView({ lat: selectedPoint.latitude, lng: selectedPoint.longitude, altitude: 2 }, 1000);
    }
  }, [selectedPoint]);
      ...
      <div className="absolute top-2 right-2 md:top-4 md:right-4 bg-black bg-opacity-80 rounded-lg p-1 md:p-2 z-10">
        <div className="flex flex-col space-y-1">
          <div className="flex space-x-1 md:space-x-2">
            <button onClick={() => { if (globeEl.current) { onPointClick?.(null); globeEl.current.pointOfView({ lat: 0, lng: 0, altitude: 2.5 }, 1000); } }} className="px-2 md:px-3 py-1 rounded text-xs font-mono bg-gray-700 text-cyan-300 hover:bg-gray-600">Reset</button>
            <button onClick={toggleAutoRotation} className={`px-2 md:px-3 py-1 rounded text-xs font-mono ${ isAutoRotating ? 'bg-green-700 text-green-200' : 'bg-gray-700 text-cyan-300' }`}>{isAutoRotating ? 'Stop' : 'Start'}</button>
          </div>
          <div className="text-xs text-center"><span className={`${ selectedPoint ? 'text-orange-300' : isAutoRotating ? 'text-green-300' : 'text-gray-400' }`}>{selectedPoint ? '🎯 Focused' : isAutoRotating ? '🔄 Rotating' : '⏸️ Paused'}</span></div>
        </div>
      </div>
      {selectedPoint && <div className="absolute top-20 left-2 md:left-4 bg-black bg-opacity-90 text-white p-3 rounded-lg border border-cyan-400 max-w-xs z-10"><div className="flex items-center justify-between mb-2"><h3 className="font-bold text-cyan-300">{selectedPoint.name}</h3><button onClick={() => onPointClick?.(null)} className="text-gray-400 hover:text-white ml-2">&#x2715;</button></div>{selectedPoint.description && <p className="text-sm text-gray-300 mb-2">{selectedPoint.description}</p>}<div className="text-xs text-cyan-400 space-y-1"><div>📍 {selectedPoint.latitude.toFixed(4)}°, {selectedPoint.longitude.toFixed(4)}°</div><div className={`inline-block px-2 py-1 rounded text-xs font-medium ${ selectedPoint.status === 'green' ? 'bg-green-600 text-green-100' : selectedPoint.status === 'yellow' ? 'bg-yellow-600 text-yellow-100' : 'bg-red-600 text-red-100' }`}>{selectedPoint.status.toUpperCase()}</div></div></div>}
      {dimensions.width > 0 && (
        <Globe
          ref={globeEl}
